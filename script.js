...
function safeUrl(url) {
    ...
    return /^(https?:\/\/|mailto:|tel:|\/|\.\/|\.\.\/)/.test(u) ? u : '#';
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const DEFAULT_NEGOCIOS = [
        ...
        {
            id: 5,
            nombre: "Distribuidora de Productos para Limpieza — Don Luis",
            categoria: "servicio",
            emoji: "🧴",
            descripcion: "Distribución de productos de limpieza para su vehículo, residencia, instituciones, hoteles, fábricas y oficinas. Propietario: Don Luis Murillo.",
            descripcion_en: "Cleaning products distributor for vehicles, homes, institutions, hotels, factories and offices. Owner: Don Luis Murillo.",
    ...
    /* Valida URLs — solo permite http/https, mailto, tel o rutas locales seguras */
    function safeUrl(url) {
        if (!url) return '#';
        const u = String(url).trim();
        return /^(https?:\/\/|mailto:|tel:|\/|\.\/|\.\.\/)/.test(u) ? u : '#';
    ...
    function renderNegocios() {
        ...
        filtered.forEach(negocio => {
            ...
            const safeImages = Array.isArray(negocio.imagenes)
                ? negocio.imagenes.map(safeAssetUrl).filter(Boolean)
                : [];
            const tieneGaleria = safeImages.length > 0;
            const safeLogo = safeAssetUrl(negocio.logo);
            const tieneBannerLogo = negocio.bannerLogo && safeLogo;
